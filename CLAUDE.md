...
- **Trigger diffing** — `updateBatch` re-runs a derivation only when a trigger field changed (`shoppingTriggers`, `scheduleTriggers`); editing the batch name doesn't rebuild the schedule. `createBatch` instead builds `batch.brewable` first — a deep clone of the recipe's own brewable for a **user** recipe, `buildBrewable(recipe)` for a **kb** one (which has none) — then runs the pipeline once: `_updateRecipe` → `_updateShopping` → `_updateSchedule`. Batch screens still read only the legacy fields; nothing renders from `batch.brewable` yet.
**Invariants.** _None._
