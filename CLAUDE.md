...
**Where.** `data/{grains,hops,yeasts,recipes}/*.json` (one file per item), `bin/build-json.js` (builder), `dist/<resource>.json` (built), `src/models.ts` (`Kb*` types), `src/brewable.ts` (`KbBrewable`/`KbBrewablePhase`/`KbAssignment` — primitives-only supertypes of the app's `Brewable`/`BrewablePhase`/`Assignment`), `src/importResource.ts`, `migrations/` (see _Batch versioning & migrations_).
**Surface.** `importResource(resource)` — fetches `/kb/<resource>.json` via core's fetchClient (same-origin, relative); return type inferred from the literal resource string via `ResourceTypeMap`. `Kb*` model types (`KbRecipe`, `KbGrain`, `KbHop`, `KbYeast`, `KbScalar`) — primitives only (no enums), nesting allowed, no normalization. `KbRecipe.brewable: KbBrewable` carries the recipe's own schedule + assignments straight from data — the app narrows it into its own `Brewable` shape via `kbBrewableToBrewable` rather than deriving one at runtime (see _Model boundary_ in `packages/app`).
...
**Gotchas.** `model/recipe.ts` is now the live editable app model for user-owned recipes — `Recipe extends Entity`, built on app `Scalar`/models (the editable complement to `KbRecipe`), versioned via `RECIPE_MODEL_VERSION` and persisted through `storage/recipes.ts` + `state/recipes.ts`. KB-sourced recipes still flow as `KbRecipe` everywhere via `state/kbRecipes.ts`, untransformed. `model/checklist-definition.ts` is now orphaned (only `model/recipe.ts` imported it, and that import was dropped in the rewrite). `model/brewable.ts` defines a third family — `Brewable` (`schedule`+`assignments`), the shared schedule/assignments structure a brew-day pilot is being built on; app-local for now (the core/kb long-term home is an open question). `Recipe.brewable: Brewable`, `Batch.brewable: Brewable`, and `KbRecipe.brewable: KbBrewable` all carry one now — no derivation, all narrowing. `defaultRecipe` seeds a brand-new `Recipe`'s with `defaultBrewable()`; `kbRecipeToRecipe` instead narrows the kb data's own `kbRecipe.brewable` via `transform/kbBrewableToBrewable.ts` (a cast-based narrowing of loose kb strings into the app's `PhaseType`/`ResourceType`/discriminated `Assignment`, mirroring `kbRecipeHopsToHops`), backstopped by an ensure-on-load shim in `state/recipes.ts` for recipes saved before the field existed (see _Batch versioning & migrations_). `createBatch` builds `batch.brewable` the same way: a deep clone of the recipe's own brewable for a **user** recipe, `kbBrewableToBrewable(kbRecipe.brewable)` for a **kb** one (see _Derived batch data_). The now-dead runtime derivation (`transform/buildBrewable.ts`, which built a `Brewable` from the legacy arrays) has been deleted; only its `HOP_PHASE_TO_PHASE_TYPE` constant (`hop.phase` → `phaseType`: `boil→boil, secondary→ferment, dry→ferment`) survived the retirement, relocated to `model/brewable.ts` since `_projectRecipeBrewable` still needs it in reverse. The **reverse** direction is wired: **the brewable is the editing source of truth** once a screen writes to it, and `saveRecipe` (`state/recipes.ts`) calls `actions/_projectRecipeBrewable.ts` on every save to rebuild `recipe.{grains,hops,yeasts,additives,mash,boil,equipment}` from `recipe.brewable`, so `RecipeOverview` and `createBatch` — which still read the legacy arrays — keep working untouched. `screen/recipe-edit-schedule/` is the first screen writing to it: one `phases.tsx` list rendered from `recipe.brewable.schedule.phases` (replacing the old separate mash/boil/equipment sections), an add-row (`phases-add-row.tsx`) picking a `PhaseType` (`mash`/`boil`/`ferment`, repeats allowed) to append a phase, and each phase row (`phases-row.tsx`) expanding to its own per-phase equipment list (`phase-equipment-row.tsx`/`phase-equipment-add-row.tsx`, picked from `data/equipment.ts`) — all through `useJsonEdit` dot-paths into `brewable.schedule.phases[...]`. Removing the last phase of a required type is blocked via `canRemovePhase`. The **brewable is now a screen's editing source of truth** for Ingredients too: `screen/recipe-edit-ingredients/index.tsx` reads/writes `recipe.brewable.assignments` directly (no `kbXToRecipeX` legacy-array editing there anymore) — grouped by `phaseType` (outer, ordered by first appearance in `schedule.phases`) then `resourceType` (inner subsection, `grain`/`hop`/`yeast`/`additive`), with a single add-row (phase + resource type + resource, default-filled via `catalog-defaults.ts`) rather than one per resource type. Per-assignment rows (`assignment-row.tsx`) are still a **name + remove placeholder** — the resource-narrowed advanced fields are a follow-up; the old per-resource-type files (`grains.tsx`/`hops.tsx`/`yeasts.tsx`/`additives.tsx` + their `*-row.tsx`/`*-add-row.tsx`) are unreferenced now but left in place as a pattern reference for that follow-up. Grains/yeasts/additives/hops come straight from `assignments` grouped by `resourceType` (hop's `phase` field is kept coherent via the reverse of `HOP_PHASE_TO_PHASE_TYPE`, falling back to a per-phaseType default when an assignment moved phaseType without `phase` following). Equipment flattens `schedule.phases[].equipment`, deduped by name. Mash/boil have **no Brewable equivalent** (a `BrewablePhase` carries equipment, not step content) — only step *count* syncs to matching phase count; existing steps are kept by index (preserving user-entered temp/time), extra phases get a freeform default step.
**Example.** _None._
...
**Where.** `_updateShopping` → `batch.shopping`; `_updateSchedule` → `batch.schedule` (see _BatchSchedule_); `_updateRecipe(recipe, brewable, batch)` projects `brewable` (plus the recipe's mash/boil steps, which have no Brewable equivalent) onto `batch.{grains,hops,yeasts,additives,mash,boil,phases}`. `updateBatch` (edit save path), `createBatch` (instantiation).
...
- **Trigger diffing** — `updateBatch` re-runs a derivation only when a trigger field changed (`shoppingTriggers`, `scheduleTriggers`); editing the batch name doesn't rebuild the schedule. `createBatch` instead builds `batch.brewable` first — a deep clone of the recipe's own brewable for a **user** recipe, `kbBrewableToBrewable(kbRecipe.brewable)` for a **kb** one (narrowed straight from the kb data's own brewable, not derived — see _Model boundary_) — then runs the pipeline once: `_updateRecipe` → `_updateShopping` → `_updateSchedule`. Batch screens still read only the legacy fields; nothing renders from `batch.brewable` yet.
**Invariants.** _None._
**Gotchas.** ⚠️ Derived fields are **not yet migrated** (see _Batch versioning & migrations_): a batch stored before a derived field existed throws until re-derived or purged (`/?purge=true`). The standalone Checklists screen + its `_updateChecklists` derivation were **removed**; equipment checkoff moved onto BatchSchedule phases, and the batch's second tab is now `Shopping`. (`model/recipe.ts` is now the live user-recipe model — see _Model boundary_; `model/checklist-definition.ts` is orphaned dead code — its sibling `component/checklist/` (the unused `ChecklistAdd`) was deleted as dead code.) The recipe side has its own, analogous derivation — `actions/_projectRecipeBrewable.ts` rebuilds the legacy recipe arrays from `recipe.brewable` on every `saveRecipe` — see _Model boundary_ for the field-by-field mapping.
**Example.** _None._
