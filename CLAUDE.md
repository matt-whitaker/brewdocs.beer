...
npm test -w packages/app          # eslint — the verification gate (see Linting below)
```
...
- **`app` is linted** (eslint 9 flat config, `packages/app/eslint.config.js`). Ratchet policy: **errors block, warnings inform** — `npm test` exits 0 while warnings remain (currently only `react-refresh/only-export-components`, left as warnings on purpose; the earlier `react-hooks/exhaustive-deps` batch was resolved). Beyond the type/hooks recommended sets, the config enforces **style + import conventions**: `@stylistic` (double quotes, semicolons, 4-space indent), `import-x/order` (external → `@brewdocs.beer/*` → `@/` → relative, alphabetized) + `import-x/no-relative-packages`, and `no-restricted-imports` banning `lodash` and `../` parent-relative imports (use the `@/` alias). Those import rules are **scoped to `src/**`** — build/script files at the package root (`vite.config.ts`, `migrations/*`) legitimately reach into sibling packages by relative path. `core`/`design` carry their own eslint configs, not part of any enforced flow.
...
### Routing
TanStack Router, file-based under `src/routes/` — `routeTree.gen.ts` is generated by the router Vite plugin; never hand-edit it. Routes: `/`, `/batches`, `/batch/$batchId`, `/recipes`, `/recipe/$recipeId`, `/recipe/$recipeId_/edit` (delisted — nothing links to it), `/knowledge`, `/disclaimer`. Path params via `Route.useParams()`. Router `defaultErrorComponent` (in `main.tsx`) renders thrown errors from suspense fetchers.
...
- `_updateRecipe(recipe, batch)` seeds ingredient fields from a `KbRecipe`, so it takes the recipe alongside the batch.
...
- **Trigger diffing.** `updateBatch` (the save path for edits) re-runs a derivation only when one of its trigger fields actually changed (`shoppingTriggers`, `scheduleTriggers`) — editing the batch name doesn't rebuild the schedule. `createBatch` instead runs the whole pipeline once, in order: `_updateRecipe` → `_updateShopping` → `_updateSchedule`.

Derived fields are **not yet migrated**: a batch stored before a derived field existed throws until re-derived or purged (`/?purge=true`). A versioning framework now exists (see *Batch versioning & migrations* below) but is scaffolding — it doesn't yet transform stored batches on load. The standalone Checklists screen and its `_updateChecklists` derivation were **removed**; equipment checkoff moved onto BatchSchedule phases (below), and the batch's second tab is now `Shopping` (the old `Checklists`-labelled-`<BatchShopping>` misnomer is fixed). `model/recipe.ts` + `model/checklist-definition.ts` remain in the tree, unused, reserved for the future user-created-recipes feature.

### Batch versioning & migrations

A version-migration framework exists, but **does not yet transform stored batches on load** — it's wired infrastructure with stub migrations, not a live migration path.

- **`core/src/migration.ts`** is the generic engine: `Migration<T>` (`{namespace, from, to, up}`), `resolveVersion` (missing `version` ⇒ v0), `buildMigrationPlan` (groups by namespace, enforces single-step `to === from+1` with no gaps), and `migrate` (walks the plan from the data's current version). Environment-agnostic — consumed by kb, app, and Vite tooling alike, so no Node/DOM APIs.
- **Per-package migrations live outside `src`**: `packages/kb/migrations` and `packages/app/migrations` (e.g. `app/migrations/batch.ts` — the `app.batch` v0→v1 step, a stub that only stamps `version: 1`). `Batch` carries a `version` field seeded from `BATCH_MODEL_VERSION` (`model/batch.ts`): **bump that constant when a stored batch would no longer parse/derive correctly, and add the matching `up` migration.**
- **Build-time plan embedding**: `vite.config.ts`'s `migrationPlanPlugin` combines kb + app migrations at build and injects the serialized plan into `index.html` as a `<script id={MIGRATION_PLAN_ELEMENT_ID} type="application/json">`; `utils/migrationPlan.ts`'s `getMigrationPlan()` reads it back at runtime. (This is why `vite.config.ts` imports `../core/src/migration` / `../kb/migrations` / `./migrations` by relative path — the eslint import rules are `src`-scoped precisely to permit it.)
- **Not yet applied**: `getMigrationPlan()` has no caller, so stored batches still throw-until-purged as today. The framework is in place for the next batch-shape change; until then, `/?purge=true` remains the reset.

### BatchSchedule screen: configurable phases

The BatchSchedule tab is driven by two batch fields:

- **`batch.phases: Phase[]`** — *configuration*: each phase is one sub-tab, rendered with the `compact` PanelSwitcher variant. A phase's `tags` is an **intersection filter** over schedule items (`["boil"]` = the whole boil; `["boil","hops"]` = only its hop additions; `[]` = everything). Phases also carry `equipment: ScheduleItem[]` (each tagged `[phase, "equipment"]`) — the kit to ready before the step, checked off in place and **user-managed from BatchPlanning**; equipment lives on the phase, not in `batch.schedule`, and its `path` is unused (`""`). Seeded by `defaultBatch` (`data/defaultBatch.ts`), whose default three phases (Mash, Boil, Ferment — Chill folded into Boil) pull their equipment from the `data/equipment.ts` catalog by `EquipmentUses`.
...
- Don't hand-edit generated files (`routeTree.gen.ts`) and don't add `lodash` (the repo deliberately uses hand-rolled `utils/func.ts` — now enforced by a `no-restricted-imports` lint rule, as is `@/`-not-`../` for intra-app imports).
...
- Defined in `.github/workflows/claude.yaml`. Two ways in: an `@claude` mention in an issue/PR comment, an inline review comment, or a submitted PR review — **or** applying the **`claude` label to an issue**. Runs `anthropics/claude-code-action` on `sonnet` with a **tiered turn budget — 40 for label-triggered feature work, 20 for an `@claude` comment poke** (`${{ github.event_name == 'issues' && 40 || 20 }}`) — and write access to contents/PRs/issues (plus `actions: read`, so it can read a failed **Verify** run).
...
- **Deps are pre-installed; it builds before opening the PR.** The workflow runs `npm ci` as its own step (zero turns), so `node_modules` is already present — the prompt tells it *not* to run `npm ci`/`npm install`, but to run `npm run build` once before opening the PR and record the result. (An earlier config with no `npm ci` step made it chase a phantom `downlevelIteration`/TypeScript error for eleven turns, because the runner's global `tsc` differed from the repo-pinned one. That's also why the prompt says a denied tool call is settled — don't re-quote and retry — and hands it the `$`-in-route-filename escape directly.)
