...
**Purpose.** CSS-first config, no tailwind.config file, no PostCSS. **design owns the shared tokens** — the daisyui `nord` theme declaration and `--font-sans` live in `packages/design/src/tokens.css`; app and www each `@import "@brewdocs.beer/design/src/tokens.css";` it by bare specifier rather than declaring their own copies. App-only tokens (SRM `--color-beer-*`, `--breakpoint-xs`, `--text-2xs`, the `--radius-selector` override, `--leading-11/12`, `.dg-collapsed ~ *`) stay in `packages/app/src/styles.css` — only what's byte-identical across consumers has moved so far.
