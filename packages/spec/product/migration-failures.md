...
**Purpose.** Where a brewer, or whoever is helping them, can see and act on records that failed
to update to the app's current data shape — reached only by direct URL, never linked to from
anywhere else in the app.

## Behaviours

**MIGRATION-FAILURES-01** — Visiting the page lists every record that failed to update to the
app's current data shape, showing enough to identify each one (what kind of record it is, and
its id when it has one) and the reason it failed.
...
> *Why:* one unrenderable record shouldn't make every other failed record invisible too.
