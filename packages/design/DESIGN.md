...
- **Tokens live in `app`, not `design`.** Every custom token documented above (`--color-beer-*`, `--breakpoint-xs`, `--leading-11`/`--leading-12`, `--text-2xs`, the `--radius-selector` override) is defined in `packages/app/src/styles.css`, not in this package. `design` only *consumes* these tokens through the class strings its components emit; it doesn't own or export them. `packages/www/src/styles/global.css` only mirrors the font and theme name — it has none of the SRM scale, the custom breakpoint, or the type tokens, so any `design` component leaning on those (the SRM `bg-beer-*` classes, `text-2xs`) would render unstyled if used from `www` today. Moving tokens into `design` itself is out of scope here — that's the subject of the separate Tailwind-consolidation research issue.
