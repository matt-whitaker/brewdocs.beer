...
**Where.** `src/models.ts` (`Entity {id}`, `Units`/`Currencies` enums — see [MODELS.md](MODELS.md) for the field-by-field reference), `src/props.ts` (`PropsWithClass` etc.), `src/event.ts` (`eventValue`), `src/fetchClient.ts` (`createFetchClient`), `src/gravity.ts` (`toSpecificGravity`, `estimateAbv`). Also `eslint.config.base.js` at the package root — the shared eslint flat-config base every workspace inherits — and `eslint-rules/no-superfluous-comments.js`, the local rule enforcing the root `CLAUDE.md` comment policy (top-level principle comments, JSDoc on exported API members, and lint directives survive; narration inside bodies errors). Both are dev tooling, not part of `src`; see _Linting_ (`packages/app/CLAUDE.md`).
